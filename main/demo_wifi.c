// main/demo_wifi.c —— WiFi 设置页：蓝牙(BLE)配网
// 流程: OK 进入配网 → 停 WiFi 释放内存 → BLE 广播 → 手机 nRF Connect 写入凭证 → 连接 WiFi
#include "demo.h"
...
static volatile prov_phase_t s_phase = PH_BLE_WAIT;
...
void demo_wifi_key(bsp_btn_t btn, bsp_btn_ev_t ev) {
    if (s_state == STATE_PROV) {
        if (btn == BSP_BTN_OK && ev == BSP_BTN_LONG) {
            if (s_phase == PH_BLE_WAIT) {
                // BLE 等待阶段可安全取消(任务 300ms 内退出)
                s_cancel = true;
                destroy_prov();
                show_info_page();
            }
