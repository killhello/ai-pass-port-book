...
```bash
python $IDF_PATH/components/spiffs/spiffsgen.py 0x630000 spiffs_data build/spiffs.bin \
...
# 刷写 SPIFFS 数据分区 (偏移 0x1D0000)
esptool.py -p COM3 -b 460800 write_flash 0x1D0000 build/spiffs.bin
```
...
# 3. 只刷写 SPIFFS 分区
esptool.py --chip esp32c3 -p COM3 write_flash 0x1D0000 build/spiffs.bin
```
...
| spiffs | data | 0x1D0000 | ~6700 KB | 电子书 + 字体 + 开机动画 |
...
4. 执行 `esptool.py write_flash 0x1D0000 build/spiffs.bin` 刷写
