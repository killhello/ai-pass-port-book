...
# 刷写 SPIFFS 数据分区 (偏移 0x1F0000)
esptool.py -p COM3 -b 460800 write_flash 0x1F0000 build/spiffs.bin
```
...
idf.py spiffsgen
# 3. 只刷写 SPIFFS 分区
esptool.py -p COM3 write_flash 0x1F0000 build/spiffs.bin
```
...
| factory | app | 0x10000 | 1920 KB | 主应用固件 |
| spiffs | data | 0x1F0000 | ~6208 KB | 电子书存储 |
...
4. 执行 `esptool.py write_flash 0x1F0000 build/spiffs.bin` 刷写
